...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        textView1.setText(String.format("%s%s", bread.name(), bread.calories()));

        bread = breadFactory.getBread("ROL");
        textView2.setText(String.format("%s%s", bread.name(), bread.calories()));

        bread = breadFactory.getBread("BRI");
        textView3.setText(String.format("%s%s", bread.name(), bread.calories()));
    }
}
